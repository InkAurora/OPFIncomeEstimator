...
loss. One constant, fitted on customers the quantile model never saw, widens both so the coverage
guarantee holds in finite samples. See
...
withhold itself and still count. It is retained as a research result and as the frozen fixed-band
comparison ADR 0007 measures sharpness against.
